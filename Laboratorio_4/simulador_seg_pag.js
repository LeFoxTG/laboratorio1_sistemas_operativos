...
/* ═══════════════════════════════════════════════
   ...
   Mismos programas del simulador del Excel realizado en clase.
...
function renderMemoryMap() {
  const MAP_H = 360; // altura total del mapa en px
...
function renderSegMap(mapH, addrCol, memCol, legendCol) {
  ...
  for (const inst of state.instances) {
    for (const seg of inst.segments) {
      blocks.push({
        base: seg.base, size: seg.size,
        color: inst.color, label: `${inst.id}·${seg.segName}`,
        type: 'inst', instId: inst.id,
      });
    }
  }
  ...
  for (const b of sortedDesc) {
    const h = Math.max((b.size / RAM) * mapH, 4);
    const blk = document.createElement('div');
    blk.className  = 'mem-block';
    blk.style.height     = h + 'px';
    blk.style.background = b.color;
    blk.style.color      = b.type === 'free' || b.type === 'so' ? '#aaa' : 'rgba(255,255,255,0.9)';
    blk.style.fontSize   = '8px';

    if (h >= 12) {
      const lbl = document.createElement('div');
      lbl.className   = 'mem-block-lbl';
      lbl.textContent = b.type === 'free' ? (h > 20 ? 'LIBRE' : '') : b.label;
      blk.appendChild(lbl);
    }
  ...
  for (const inst of state.instances) {
    ...
    item.innerHTML = `
      ...
      <div>${inst.id} ${inst.name}</div>
    `;
    legendCol.appendChild(item);
  }

  if (state.instances.length === 0) {
    legendCol.innerHTML = '<div class="text-muted text-sm">Sin instancias.</div>';
  }
}

/**
 * Mapa de Paginación:
 * Muestra cada marco como un bloque de altura proporcional al tamaño de página.
 * Coloreado según la instancia asignada o gris si está libre.
 */
function renderPageMap(mapH, addrCol, memCol, legendCol) {
  const ps         = state.pageSize;
  const totalFrames = state.frames.length;

  // Solo mostramos un subconjunto de marcos si hay demasiados
  // (para que el mapa no sea microscópico). Máximo ~100 visibles.
  const maxVisible = 100;
  const step       = Math.max(1, Math.ceil(totalFrames / maxVisible));
  const visible    = state.frames.filter((_, i) => i % step === 0);

  const blockH = Math.max(mapH / visible.length, 3);
  ...
  // Marcos de abajo a arriba (mayor dirección → menor dirección)
  const visDesc = [...visible].reverse();
  for (const frame of visDesc) {
    const inst  = frame.instanceId && frame.instanceId !== 'SO'
      ? state.instances.find(i => i.id === frame.instanceId)
      : null;

    const color = frame.instanceId === 'SO' ? '#e2e4ea'
                : !frame.free && inst       ? inst.color
                : '#f0f1f5';
    ...
    blk.style.color      = inst ? 'rgba(255,255,255,0.85)' : '#bbb';
    blk.style.fontSize   = '8px';

    if (blockH >= 12 && inst) {
      const lbl = document.createElement('div');
      lbl.className   = 'mem-block-lbl';
      lbl.textContent = `${inst.id}·P${frame.pageNo}`;
      blk.appendChild(lbl);
    }
  ...
  for (const inst of state.instances) {
    ...
    item.innerHTML = `
      <div class="legend-dot" style="background:${inst.color}"></div>
      <div>${inst.id} ${inst.name}<br>
        <span class="text-muted">${inst.pages} pág.</span>
      </div>
    `;
    legendCol.appendChild(item);
  }
